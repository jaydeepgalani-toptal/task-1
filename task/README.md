...
- [task/environment/rollup/src/registry.mjs](/workspace/task/environment/rollup/src/registry.mjs) loads each partner’s `profile`, `eventIdPath`, `sourceTsPath`, `accountPath`, and `settlement` metadata.
...
- `beta`: `eventIdPath=eventId`, `sourceTsPath=record.eventTs`, `accountPath=record.accountRef`, `settlement.zone=Europe/London`, `settlement.cutoffLocal=17:00`
- `delta`: `eventIdPath=eventId`, `sourceTsPath=mirror.eventTs`, `accountPath=mirror.accountRef`, `settlement.zone=America/Los_Angeles`, `settlement.cutoffLocal=14:30`

`alpha` and `gamma` use different profiles and act as controls.

## Actual Failure Mode

The planted defect is that [task/environment/rollup/src/payload-access.mjs](/workspace/task/environment/rollup/src/payload-access.mjs) builds the compiled resolver cache key from `partner.profile` only.
...
3. Trace how `beta` and `delta` share `ledger-wire-v2` in the registry while differing on effective resolver inputs such as `eventIdPath`, `sourceTsPath`, `accountPath`, `settlement.zone`, and `settlement.cutoffLocal`.
...
- The verifier also mutates `delta`’s `eventIdPath` in a same-process replay with a fixture copy that creates a real duplicate only under the mutated event-id reader. That check catches cache keys that ignore compiled path-reader inputs.
